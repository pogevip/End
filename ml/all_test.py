...
class CaseStatutesRecommend:
    ...
    def __rank(self, token, caseReason, SimCaseCandidateSet, alpha=0.3):
        ...
        for case in SimCaseCandidateSet:
            ...
            point = model.predict([X1, X2])

            sim = alpha*tfidf + (1-alpha)*point

            refs = [ref['name']+'-'+ref['levelone'] for ref in refs]
            res.append([id, sim, refs])

        return res

    @TimeCost('----输出')
    def __out_cases_statutes(self, rankRes, b=0.5):
        tmp = list(filter(lambda x: x[1] > b, rankRes))
        tmp.sort(key=lambda x: x[1], reverse=True)

        case_list = []
        statutes_list = defaultdict(lambda:0)
        for item in tmp:
            case_list.append(item[0])
            for ref in item[2]:
                statutes_list[ref]+=1

        statutes_list = [[k, v] for k, v in dict(statutes_list).items()]
        tmp.sort(key=lambda x: x[1], reverse=True)
