...
class TrainData():
    def __init__(self, src_path):
        ...
        self.data['is_gen'] = 0
        self.res = pd.DataFrame([], columns=['token', 'cls', 'is_gen', 'len'])

    def sample(self, base_line):
        # 对少数数据进行采样
        ...
        for cls, group in self.data.groupby('cls'):
            ...
            if len(group) < base_line:
                ...
                buffer = []
                multiple = int(math.ceil(base_line / len(tmp)))
                for _, row in tmp.iterrows():
                    buffer.append([row['token'], row['cls'], row['is_gen'], row['len']])
                    for i in range(multiple):
                        token = process_token(row['token'])
                        buffer.append([token, row['cls'], 1, len(token.split(' '))])

                df = pd.DataFrame(buffer, columns=['token', 'cls', 'is_gen', 'len'])
                df = df.sample(base_line)
                self.res = pd.concat([self.res, df], ignore_index=True)
            elif len(group) > base_line * 5:
                if len(group[(group['len'] <= 200) & (group['len'] >= 20)]) > base_line * 5:
                    tmp = group[(group['len'] <= 200) & (group['len'] >= 20)].sample(base_line * 5)
                else:
                    tmp = group[(group['len'] <= 200) & (group['len'] >= 20)]
                    tmp = tmp.sample(len(tmp) // 10000 * 10000)
                self.res = pd.concat([self.res, tmp], ignore_index=True)
            else:
                tmp = group[(group['len'] <= 300) & (group['len'] >= 20)]
                tmp = tmp.sample(len(tmp) // 10000 * 10000)
                self.res = pd.concat([self.res, tmp], ignore_index=True)
    ...
    def split_train_val_test_set(self):
        ...
        for _, group in self.res.groupby('cls'):
            ...
            test_index = group.sample(num).index
    ...
    def run(self, min_num=50000):
        self.sample(base_line=min_num)
