...
from classifier_tensorflow.TextCNN import TextCNN
...
def DataGenerator(x, y, epoch, batch_size, shuffle=True):
    def stuff_doc(doc):
        doc = doc * math.ceil(max_sequence_lenth/len(doc))
        return doc[:max_sequence_lenth]
    ...
    num_batches_per_epoch = math.ceil(len(x)/batch_size)
    ...
    for _ in range(epoch):
        ...
        for batch_index in range(int(num_batches_per_epoch)):
            ...
            batch_data_x = [x[k] for k in batch_indexs]
            batch_data_y = [y[k] for k in batch_indexs]
...
def train(train_file_path, val_file_path, vocab_dic_path):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            batches = DataGenerator(train_x, train_y, batch_size, num_epochs)
