...
class HanTfrecordGenerator(TfrecordGenerator):
    ...
    def process_data(self):
        def helper(doc):
            ...
            sents = map(
                lambda sent: list(filter(lambda x: isinstance(x.strip(), str) and len(x.strip()) > 0, sent.split(' '))),
                sents)
            return list(sents)
...
class TextCnnTfrecordGenerator(TfrecordGenerator):
    ...
    def process_data(self):
        def helper(doc):
            doc = doc.replace('。', ' ')
            doc = filter(lambda x: isinstance(x.strip(), str) and len(x.strip())>0, doc.split(' '))
            return list(doc)
